...
def open_sms_campaign_wizard(tree, mode="add", campaign=None):
    ...
    def go_next():
        ...
        if idx == 0:
            ...
            send_sms_wizard(dialog, send_tree, sel_contact_ids, name_var.get(), message_text.get("1.0", tk.END), progress, counter_var, timer_var)
    def go_prev():
        idx = current_step[0]
        if idx > 0:
            show_step(idx-1)
...
def import_contacts_dialog(tree):
    ...
    class ImportDialog(simpledialog.Dialog):
        def body(self, master):
            tk.Label(master, text="CSV File Path:").grid(row=0, column=0, padx=5, pady=10)
            self.file_entry = tk.Entry(master, width=50)
            self.file_entry.grid(row=0, column=1, padx=5, pady=10)
            browse_btn = tk.Button(master, text="Browse", command=self.browse_file)
            browse_btn.grid(row=0, column=2, padx=5, pady=10)
            # Group selection
            tk.Label(master, text="Assign to Groups:").grid(row=1, column=0, padx=5, pady=10, sticky="ne")
            ...
            for row in c.fetchall():
                self.groups_listbox.insert(tk.END, row[0])
            conn.close()
        ...
        def apply(self):
            self.filepath = self.file_entry.get().strip()
            self.selected_groups = [self.groups_listbox.get(i) for i in self.groups_listbox.curselection()]

    dialog = ImportDialog(tree.master, title="Import Contacts from CSV")
    if not hasattr(dialog, 'filepath') or not dialog.filepath:
        return
    ...
    if not required_cols.issubset(df.columns.str.lower()):
        messagebox.showerror("Error", "CSV must have columns: name, email, mobile")
        return
    # Normalize columns
    ...
    for _, row in df.iterrows():
        try:
            c.execute("INSERT OR IGNORE INTO contacts (name, email, mobile) VALUES (?, ?, ?)",
                      (row["name"], row["email"], row["mobile"]))
            if c.rowcount:
                added += 1
            # Assign to groups
            if group_ids:
                c.execute("SELECT id FROM contacts WHERE email=?", (row["email"],))
                contact_row = c.fetchone()
                if contact_row:
                    contact_id = contact_row[0]
                    for gid in group_ids:
                        c.execute("INSERT OR IGNORE INTO group_members (group_id, contact_id) VALUES (?, ?)", (gid, contact_id))
        except Exception:
            continue
    conn.commit()
    conn.close()
    load_contacts_with_checkboxes(tree, lambda values, sn: tree.insert("", tk.END, values=("", sn) + values, tags=("unchecked",)))
    messagebox.showinfo("Import Complete", f"{added} contacts added.")

def show_campaigns(parent):
    # Clear existing content
    for widget in parent.winfo_children():
        widget.destroy()

    # Toolbar for campaigns
    toolbar = ttk.Frame(parent)
    toolbar.pack(fill=tk.X, pady=(0, 10))
    btn_group = ttk.Frame(toolbar)
    btn_group.pack(side=tk.LEFT)
    add_btn = ttk.Button(btn_group, text="Add Campaign", command=lambda: add_campaign(campaigns_tree))
    add_btn.pack(side=tk.LEFT, padx=(0, 2))
    edit_btn = ttk.Button(btn_group, text="Edit Campaign", command=lambda: edit_campaign(campaigns_tree))
    edit_btn.pack(side=tk.LEFT, padx=2)
    delete_btn = ttk.Button(btn_group, text="Delete Campaign", command=lambda: delete_campaign(campaigns_tree))
    delete_btn.pack(side=tk.LEFT, padx=2)

    # Campaigns list
    columns = ("Name", "Subject", "Body")
    campaigns_frame = ttk.Frame(parent)
    campaigns_frame.pack(fill=tk.BOTH, expand=True)
    campaigns_tree = ttk.Treeview(campaigns_frame, columns=columns, show="headings", height=10)
    # Load saved widths
    col_widths = load_column_widths("campaigns", columns)
    campaigns_tree.heading("Name", text="Campaign Name")
    campaigns_tree.column("Name", width=col_widths.get("Name", 180))
    campaigns_tree.heading("Subject", text="Subject")
    campaigns_tree.column("Subject", width=col_widths.get("Subject", 220))
    campaigns_tree.heading("Body", text="Body")
    campaigns_tree.column("Body", width=col_widths.get("Body", 300))
    campaigns_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
    # Scrollbar (vertical, to the right of the tree)
    scrollbar = ttk.Scrollbar(campaigns_frame, orient=tk.VERTICAL, command=campaigns_tree.yview)
    scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
    campaigns_tree.configure(yscrollcommand=scrollbar.set)

    load_campaigns(campaigns_tree)

    def on_column_resize(event):
        save_column_widths("campaigns", campaigns_tree)
    campaigns_tree.bind("<ButtonRelease-1>", on_column_resize)

def load_campaigns(tree):
    for item in tree.get_children():
        tree.delete(item)
    import sqlite3
    conn = sqlite3.connect(DB_FILE)
...
def add_campaign(tree):
    open_campaign_wizard(tree, mode="add")

def edit_campaign(tree):
    selected = tree.selection()
    if not selected or len(selected) != 1:
        messagebox.showinfo("Edit Campaign", "Please select exactly one campaign to edit.")
        return
    iid = selected[0]
    ...
    if not row:
        ...
        return
    campaign_id = row[0]
    c.execute("SELECT contact_id FROM campaign_contacts WHERE campaign_id=?", (campaign_id,))
    contact_ids = [r[0] for r in c.fetchall()]
    conn.close()
    open_campaign_wizard(tree, mode="edit", campaign={
        'id': campaign_id,
        'name': old_name,
        'subject': old_subject,
        'body': old_body,
...
def delete_campaign(tree):
    selected = tree.selection()
    if not selected or len(selected) != 1:
        messagebox.showinfo("Delete Campaign", "Please select exactly one campaign to delete.")
        return
    iid = selected[0]
    name = tree.item(iid, "values")[0]
...
# --- Send Emails Wizard ---
def send_emails_wizard(dialog, send_tree, contact_ids, campaign_name, subject, body, progress, counter_var, timer_var):
    ...
    def send_thread():
        nonlocal success, failed
        import email_utils
        for idx, (cid, cname, cemail) in enumerate(contacts):
            scroll_to_row(idx)
            send_tree.item(send_tree.get_children()[idx], tags=("current",))
            dialog.update_idletasks()
            subj = subject.replace("{{name}}", cname).replace("{{email}}", cemail)
            body_val = body.replace("{{name}}", cname).replace("{{email}}", cemail)
            try:
                if email_method == 'smtp':
                    smtp_settings = {"server": smtp_server, "port": smtp_port}
                    email_utils.send_email('smtp', smtp_settings, sender, password, cemail, subj, body_val)
                elif email_method == 'sendgrid':
                    email_utils.send_email('sendgrid', {"sendgrid_api_key": sendgrid_api_key}, sender, None, cemail, subj, body_val)
                elif email_method == 'ses':
                    email_utils.send_email('ses', {"ses_access_key": ses_access_key, "ses_secret_key": ses_secret_key, "ses_region": ses_region}, sender, None, cemail, subj, body_val)
                send_tree.set(send_tree.get_children()[idx], column="Status", value="✔️")
                success += 1
            except Exception as e:
                send_tree.set(send_tree.get_children()[idx], column="Status", value="❌")
